...
class Flickr25k(Dataset):
    ...
    def init(root, num_query, num_train):
        ...
        """
        # Load dataset
        img_txt_path_d = os.path.join(root, 'db_image.txt')
        target_txt_path_d = os.path.join(root, 'database_label.txt')

        img_txt_path_t = os.path.join(root, 'test_image_m.txt')
        target_txt_path_t = os.path.join(root, 'test_label.txt')

        # Read files
        with open(img_txt_path_d, 'r') as f:
            data_d = np.array([i.strip() for i in f])
        with open(img_txt_path_t, 'r') as f:
            data_t = np.array([i.strip() for i in f])
        targets_d = np.loadtxt(target_txt_path_d, dtype=np.int64)
        targets_t = np.loadtxt(target_txt_path_t, dtype=np.int64)

        data = np.concatenate([data_d, data_t])
        targets = np.concatenate([targets_d, targets_t])


        # Split dataset
        # perm_index = np.random.permutation(data_d.shape[0])
        # train_index = perm_index[: num_train]

        # Flickr25k.QUERY_DATA = data_t
        # Flickr25k.QUERY_TARGETS = targets_t
        #
        # Flickr25k.TRAIN_DATA = data_d[train_index]
        # Flickr25k.TRAIN_TARGETS = targets_d[train_index, :]
        #
        # Flickr25k.RETRIEVAL_DATA = data_d
        # Flickr25k.RETRIEVAL_TARGETS = targets_d
